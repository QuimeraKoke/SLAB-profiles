...
def build_roster(category) -> dict:
    ...
    acwr = _player_acwr(category, pids)
...
def _wellness_and_forma(category, pids: list) -> tuple[dict, dict]:
    ...
    normalized) + a recent-trend sparkline per player. Forma is
    oldest→newest."""
    from api import wellness as w

    fmax = w.field_max(category)
    recent = w.recent_by_player(category, pids, limit=_FORMA_POINTS)
    wellness, forma = {}, {}
    for pid, datas in recent.items():
        scores = [s for s in (w.score(d, fmax) for d in datas) if s is not None]
        if not scores:
            continue
        wellness[pid] = scores[0]  # newest
        trail = list(reversed(scores))  # oldest → newest for the sparkline
        forma[pid] = [{"value": s, "tone": _score_tone(s)} for s in trail]
    return wellness, forma


def _player_acwr(category, pids: list) -> dict:
    """Per-player ACWR = acute (7d) ÷ chronic (28d-weekly) total distance,
    matches + trainings. None when GPS is too sparse."""
    templates = list(
    ...
    for pid, ch in chronic.items():
        weekly = ch / 4.0
        if weekly > 0:
            out[pid] = round(acute.get(pid, 0.0) / weekly, 2)
    return out
